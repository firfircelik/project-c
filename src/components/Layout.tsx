...
const navItems = [
  ...
  { icon: Building2, label: 'İlanlar', path: '/listings' },
  { icon: Users, label: 'Müşteriler', path: '/customers' },
  { icon: KanbanSquare, label: 'Satış Panosu', path: '/kanban' },
  { icon: CalendarDays, label: 'Takvim', path: '/calendar' },
  { icon: FileText, label: 'Sözleşmeler', path: '/contracts' },
  { icon: PieChart, label: 'Raporlar & Prim', path: '/reports' },
];
...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans selection:bg-indigo-100 selection:text-indigo-900">
      ...
      {/* Sidebar */}
      <aside className={cn(
        "fixed md:sticky top-0 left-0 h-[100dvh] w-72 bg-white border-r border-slate-200 flex flex-col transition-transform duration-300 ease-in-out z-50 shadow-2xl md:shadow-none",
